...
public class EnemyMoveSystem : MonoBehaviour
{
    ...
    // 배열로 임시로 위치 잡아 놓는 pos들 놓고 따라가게 하기
    // 시도시 마다, 랜덤으로 섞어주기
    ArrayList movePos;
    ...
    public float turnovertime = 0;
    
    // Start is called before the first frame update
    void Start()
    {
        rigid = GetComponent<Rigidbody>();
        movePos = new ArrayList();
        enemyMoveState = 0;
        turnovertime = 0;
    }
    ...
    public void OnApproachstate(Player player)
    {
        Instantiate(player, player.playerinputSystem.playerPos, transform.rotation);
    }
}
